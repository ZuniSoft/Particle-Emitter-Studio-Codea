...
function PEmitter:init(x,y)
    ...
    self.deadRects = {}
    self.parts = {}
    self.emitterMesh = mesh()
    self.evo = 0
    self.tSinceLast = 0.0
    self.particleLimit = 50
end

function PEmitter:draw()
    collectgarbage()
    ...
    -- Calculating gravity
    local g
    if self.useGravityVector == false then
    ...
    for k,i in ipairs(self.rects) do
        ...
        if (p.lifeLeft > 0) then
            ...
            if ((self.sizeWiggle>0)and(size>1)) then
                size = math.random(math.floor(size))
            end
            if (size<0) then
                size = 0
            end
            ...
            self.emitterMesh:setRect(i,p.x,p.y,size,size,p.angle)
            self.emitterMesh:setRectColor(i,255,255,255,opacity)
        else
            local deadPart = self.parts[i]
            if not deadPart.isDead then
                table.insert(self.deadRects,i)
                self.emitterMesh:setRect(i,0,0,0,0)
                deadPart.isDead = true
            end
        end
    end
    self.emitterMesh:draw()
end

function PEmitter:createParticle()
    ...
    if (self.deadRects[1]==nil) then
        index = self.emitterMesh:addRect(self.x,
        self.y,
        psize,
        psize)
        ...
        self.emitterMesh:setRect(index,
        self.x,
        self.y,
        psize,
        psize)
    ...
    self.emitterMesh:setRectColor(index,255,255,255,
